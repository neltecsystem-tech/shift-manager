...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const coKeyAcc = (v: unknown) => String(v ?? '').normalize('NFKC')
      ...
      .toLowerCase();
    // 🚨 営業所名も入れる。non_company_names(会計側の表)に足すのを忘れると、その営業所の人の
    //    会社名が営業所名のまま確定し、支払通知書の宛名が「川越 様」になる。金額は正しいので
    //    誰も気付かない。実際に 2026-08 の本田裕太が該当し、川越が表に載ったのは 9/8 だった。
    //    表への追加は残しつつ、営業所は構造的に支払先ではないのでここを下限にする。
